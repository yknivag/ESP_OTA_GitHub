...
//const char* mqttServer = "6ed8dd8708344a93bff01c7f49786d20.s2.eu.hivemq.cloud";
...
//initial MQTT setup, if a file named mqtt-config.txt is uploaded to spiffs that contains the information in the form 
/*
...
For example, the line "mqttDevicenameTopic=MyDevice/devicename/" sets the "mqttDevicenameTopic" variable to "MyDevice/devicename/". The sketch uses this variable to subscribe to the "devicename" topic on the MQTT broker.
in summary, the "mqtt-config.txt" file allows you to customize the MQTT topics and variables used by the sketch without having to modify the code.
 */
...
//other, static MQTT definitions (they are set in sketch as they dont contain private data. Therefore its not critical to upload this code publicly on github).
...
// Valid variable names for MQTT topics in mqtt-topics.txt config file in SPIFFS memory of ESP8266
...
// MQTT topic variables efinition. Essentially, mqttVars is an array of pointers to the MQTT topics that will be read from the mqtt-topics.txt file and assigned to the respective MQTT variables.
...
/*The function read_mqtt_topics_from_configfile() reads MQTT topics from a file named "mqtt-topics.txt" stored in the SPIFFS memory of a device. 
...
void read_mqtt_topics_from_configfile(){
  // Open mqtt-topics.txt file for reading
  File MQTTconfigFile = SPIFFS.open("/mqtt-topics.txt", "r");
  ...
  if (MQTTconfigFile) {
    ...
    while (MQTTconfigFile.available()) {
      ...
      if (!foundVar) {
        Serial.println("Error: Invalid variable name '" + varName + "'. Skipping.");
      }
    }
    MQTTconfigFile.close();
    Serial.println("mqtt-topics.txt has been read from SPIFFS memory. \nThe MQTT settings are now set to:");
    ...
    Serial.println(F("Failed to open mqtt-topics.txt file. To store individual MQTT topics for the device to connect to, create a text file named 'mqtt-topics.txt', and add the MQTT topics in a form like this:"));
...
#define GHOTA_CURRENT_TAG "0.2.1"
...
//  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
      ...
      //check_ota_github(); //commented out for testing AAA DEBUG
